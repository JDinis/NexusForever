...
namespace NexusForever.WorldServer.Game.Entity
{
    public class SpellManager : IUpdate, ISaveCharacter
    {
        ...
        public SpellManager(Player owner, CharacterModel model)
        {
            ...
            foreach (CharacterSpellModel spellModel in model.Spells)
            {
            ...
            for (byte i = 0; i < ActionSet.MaxActionSets; i++)
            {
                ...
                foreach (CharacterActionSetShortcutModel shortcutModel in model.ActionSetShortcuts
                    .Where(c => c.SpecIndex == i))
                    actionSets[i].AddShortcut(shortcutModel);

                foreach (CharacterActionSetAmpModel ampModel in model.ActionSetAmps
                    .Where(c => c.SpecIndex == i))
